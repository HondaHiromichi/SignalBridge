...
| UI刷新 | デザイン刷新 (Figma 案 B v2: ダーク=コンソール風, 左サイドバー操作 + 右大型ログ) を uGUI 実装 | 完了 (基準 1920x1080, アンカーでレスポンシブ, ログ色分け RECV緑/ERROR赤, Main Camera 背景を単色ダーク化, 2026-06-12) |
| UI配線 | ログツールバー / タブ / ステータスを機能化 | 完了 (Clear=ログ消去, フィルタ すべて/SEND/RECV/ERROR=種別絞り込み, ログ/モニタ タブ切替 (モニタは Phase 2 用プレースホルダ), ステータスピルが Listen 状態連動, 2026-06-12) |
| ツールチップ | 各セクション見出しのホバーで解説 (目的+使い方) をポップ表示 | 完了 (Tooltip + TooltipTrigger, 0.5秒ホバーで表示, カーソル追従+画面内クランプ, 送信/Raw 送信/受信/プリセット/ログ の5見出しに付与, 2026-06-12) |
...
└─ SignalBridge/
    ...
    ├─ Tooltip.cs               見出しホバー時の共有ツールチップ (表示/カーソル追従/クランプ)
    ├─ TooltipTrigger.cs        見出しに付与し, ホバーで Tooltip に解説を渡す
...
Add: Wire log toolbar/tabs/status (filter, clear, monitor tab, listen pill)  <- UI 配線
Fix: Vertically center text in OscDemo input fields
Update: Restyle listen toggle as square box and align 受信 row
Fix: Make log toolbar chip/Clear buttons clickable                           <- クリック修正
Add: Section header tooltips (purpose + usage on hover)                      <- ツールチップ
```
